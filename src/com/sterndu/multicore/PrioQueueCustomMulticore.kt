@file:JvmName("PrioQueueCustomMultiCore")
...
/*
	Updater needs to be demoted to be just another taskhandler
	singleshots should be simply insertable with th = null
	need to build a structure of repeatable task & singleshot task so that -
	a scheduler thread that handles a collection queue (prioqueue) and fills thoses tasks as single shots into
	a blocking queue, from which worker threads can pull tasks

	later: replace prioqueue with independent implementation but as encryption also is implemented as jvm only this is future

 */

object PrioQueueCustomMultiCore {
	...
	var maxSimultaneousThreads = Runtime.getRuntime().availableProcessors() - 1
		private set
